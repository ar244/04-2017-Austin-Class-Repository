# 23.3 Lesson Plan - First Steps with Python
...
`Summary: Complete activities 9-12 in Unit 23`
...
* Make sure students install Python 3.4.x+ _before_ coming to class. Windows and Mac users can find installers on the [Python downloads page](https://www.python.org/downloads/release/python-352/).
  ...
  * Slack out the `InstallationInstructions` before students come to class.
...
* Open up `09-Console-IO-Python/Examples/examples.py`
...
* Slack out the `09-Console-IO-Python/Examples/examples.py` so students can refer to it as they work through the exercise.

* Slack out the `09-Console-IO-Python/Unsolved/unsolved.py` file to students. It contains instructions, so you don't need to slack these out separately.

### 4. Instructor Do: Review Activity  (0:10)

* Open up `09-Console-IO-Python/Solved/solved.py`. This activity is fairly straightforward, and is mainly intended to help students get some practice with Python's syntax.
...
* Open up `10-Lists-Dicts-Conditions-Python/Examples/list_examples.py`.
...
* Slack out the `10-Lists-Dicts-Conditions-Python/Examples/list_examples.py` file for student reference.

* Open up the `10-Lists-Dicts-Conditions-Python/Examples/dict_examples.py`.
...
* Next, point out that we can get both the key _and_ the value by using the `items` method.
  ...
  * This would be a good time to talk about how indentation works in Python.

    Point out how the `for` loop has no curly brackets, and that the code block must be indented further than the `for ...`.

    Discuss how the entire code block must be indented the same amount, although, the amount of indentation is left to the programmer.
...
* Slack out the `10-Lists-Dicts-Conditions-Python/Examples/dict_examples.py` for student reference.

* Finally, open up the `10-Lists-Dicts-Conditions-Python-Lists-Dicts-Conditions/Examples/conditionals_example.py`.
...
* Slack out the `10-Lists-Dicts-Conditions-Python/Examples/conditionals_example.py` for student reference.
...
* Slack out the `10-Lists-Dicts-Conditions-Python/unsolved.py`. The file contains instructions, so there is no need to slack them out separately.

### 7. Instructor Do: Review Activity  (0:10)

* Open up the `10-Lists-Dicts-Conditions-Python/Solved/solved.py`.
...
* Open up `11-Functions-Python/Examples/examples.py`.
...
* Slack out the `11-Functions-Python/Examples/examples.py` for student reference.

### 10.  Students Do: Refactor Student Information Tool  (0:15)

* Slack out the `11-Functions-Python/Unsolved/unsolved.py`. Instructions are included in the file, so there is no need to slack them out separately.
...
* Briefly open up the `11-Functions-Python/Solved/helpers.py`.

  * Point out that this file has a lot of function definitions. 

  8 Explain that we'll look at them in detail momentarily.

  * For now, close the file, and open up `11-Functions-Python/Solved/solved.py`
...
* Return to the `11-Functions-Python/Solved/helpers.py`.
...
* Slack out the `11-Functions-Python/Solved/solved.py` for student reference.
...
* Open up the `12-Classes-and-Objects-Python/Examples/examples.py`.
...
* Slack out the `12-Classes-and-Objects-Python/Examples/examples.py` for student reference.

### 13.  Students Do: Create a Student Class (0:25)

* Slack out the `12-Classes-and-Objects-Python/Unsolved/unsolved.py`. The file contains instructions, so there is no need to slack them out separately.

### 14.  Instructor Do: Review Activity  (0:10)

* Ask a student to explain how they implemented their `Student` class.

  * Check their response against `12-Classes-and-Objects-Python/Solved/Student.py`.
...
* Ask if anyone got as far as implementing a `Roster` class.
  ...
  * Check their response against `12-Classes-and-Objects-Python/Solved/Roster.py`.
...
* Finally, ask someone to walk through how they restructured the logic of their main program.
  ...
  * Step through the `12-Classes-and-Objects-Python/Solved/solved.py`.
